  ...
  class SaleDates < Generator
    ...
    def generate(site)
      ...
      site.config['dates'] = {
        ...
        'dropoff' =>  "#{@start_date.strftime('%A, %B %-d')}#{ordinal(@start_date.strftime('%-d'))} from 4:00 p.m. to 9:00 p.m., #{@dropoff.strftime('%A, %B %-d')}#{ordinal(@dropoff.strftime('%-d'))} from 10:00 a.m. to 1:00 p.m. *Restocking Consignors* drop-ff Wednesday, April 15th from 12 p.m. to 2 p.m.",
